...
class xinye_psaug(Dataset):
    def __init__(self, path_to_database, path_to_protocol, part='train'):
        super(xinye_psaug, self).__init__()
        self.ptd = path_to_database
        self.part = part
        self.path_to_audio = '/data7/xyk/xinye_psaug4/'
        self.path_to_protocol = path_to_protocol
        protocol = '/data7/xyk/xinye_psaug_label/train4.txt'
        self.label = {"fake": 0, "real": 1}
        with open(protocol, 'r') as f:
            audio_info = [info.strip().split() for info in f.readlines()]
            self.all_info = audio_info
...
class xinye_eval(Dataset):
    def __init__(self):
        super(xinye_eval, self).__init__()
        self.path_to_audio = os.path.join('/data7/xyk/xinye/test')
        self.audio_files = os.listdir(self.path_to_audio)
...
class asv5_eval(Dataset):
    def __init__(self, path_data, path_label):
        super(asv5_eval, self).__init__()
        self.path_to_audio = path_data
        self.path_to_protocol = path_label
    ...
    def __getitem__(self, idx):
        filename = self.all_info[idx][0] + '.flac'
        filepath = os.path.join(self.path_to_audio, filename)
        waveform, sr = torchaudio_load(filepath)
...
class asvspoof19(Dataset):
    def __init__(self,  path_to_features, path_to_protocol):
        ...
        self.processor = Wav2Vec2FeatureExtractor.from_pretrained("/data3/xyk/huggingface/wav2vec2-xls-r-300m/")    
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        waveform, sr = torchaudio_load(filepath)
        waveform = pad_dataset(waveform)
        waveform = self.processor(waveform, sampling_rate=16000,return_tensors="pt").input_values.cuda()  # torch.Size([1, 31129])
