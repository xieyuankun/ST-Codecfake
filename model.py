...
class XLSRAASIST(nn.Module):
    def __init__(self, model_dir, device='cuda'):
        ...
        self.w2vaasist = W2VAASIST()

    def forward(self, audio_data):
        # Extract features using XLSR
...
class ResNet18ForAudio(nn.Module):
    def __init__(self, enc_dim=256, nclasses=7):
    ...
    def forward(self, x):
        x = self.spec(x.cuda()).unsqueeze(dim=1)
        x = self.resnet18(x)
        x = x.view(x.size(0), -1) 
        feat = self.fc(x)
        mu = self.fc_mu(feat)
...
class MELLCNN(nn.Module):
    def __init__(self, nclasses=7):
        super(MELLCNN, self).__init__()
        self.enc_dim = 256
        ...
        self.mel = torchaudio.transforms.MelSpectrogram(n_mels=80,n_fft= 512, hop_length=128, sample_rate = 16000).cuda()
    def forward(self, x):
        x = self.mel(x.cuda()).unsqueeze(dim=1)
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.conv4(x)
